...
def load_from_folder(folder, dataset_folder, id_prefix,
    ...
    for patient_folder in patient_folders:
        ...
        for examination_folder in examination_folders:
            ...
            examination_folder = os.path.join(
                patient_folder, examination_folder)
...
def _strip_labels(train_cancer_set, keep_ratio):
    ...
    for patient_id, exam_data in train_cancer_set.items():
        for exam_id, entries in exam_data.items():
            ...
            target_num_labeled_slices = int(
                np.ceil(num_labeled_slices * keep_ratio))
...
def assign_patients(dataset_folder, train_ratio, val_ratio, test_ratio,
    ...
    if label_keep_ratio < 1.0:
        output_file = '{}_keep_{}'.format(
            output_file, int(label_keep_ratio * 100))
    ...
    assert os.path.exists(
        healthy_cases_folder), f'failed to find {healthy_cases_folder}'
    assert os.path.exists(
        cancer_cases_folder), f'failed to find {cancer_cases_folder}'
    assert os.path.exists(
        cancer_annotations_folder), f'failed to find {cancer_annotations_folder}'
    ...
    if num_healthy_patients == 0:
        ...
        patient_ratio = float(num_healthy_patients) / \
            float(num_cancer_patients)
    ...
    for patient_id, exam in train_data_dict.items():
        ...
        for exam_id, entries in exam.items():
            train_patient_exam_id_to_num_slices[patient_id][exam_id] = len(
                entries)
    ...
    for patient_id, exam in val_data_dict.items():
        ...
        for exam_id, entries in exam.items():
            val_patient_exam_id_to_num_slices[patient_id][exam_id] = len(
                entries)
    ...
    for patient_id, exam in test_data_dict.items():
        ...
        for exam_id, entries in exam.items():
            test_patient_exam_id_to_num_slices[patient_id][exam_id] = len(
                entries)
