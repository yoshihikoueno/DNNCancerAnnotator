...
def _load_existing_tfrecords(directory, split_name, target_dims, dataset_name,
    ...
    ids = dataset_info[standard_fields.PickledDatasetInfo.patient_ids][split_name]
    ...
    if dataset_name == 'prostate_cancer':
        return pc.build_tf_dataset_from_tfrecords(
            ...
            use_2d_input_architecture=use_2d_input_architecture)
    else: raise ValueError


# Create tfrecords files if necessary and return meta info as dict
def prepare_dataset(dataset_config, directory, existing_tfrecords, target_dims, seed):
    meta_file = os.path.join(dataset_config.dataset_path, dataset_config.dataset_info_file)
    ...
    success_indicator = os.path.join(output_dir, 'SUCCESS')

    if not (os.path.exists(output_dir) and os.path.exists(success_indicator)):
        ...
        # Create marker file that indicates that the tfrecords were successfully created
        Path(success_indicator).touch()
    return meta_data
...
def build_dataset(dataset_name, directory,
                  ...
                  target_depth, use_2d_input_architecture):
    ...
    if shuffle and is_training:
        dataset = dataset.apply(tf.data.experimental.shuffle_and_repeat(shuffle_buffer_size, None))
    elif is_training:
    ...
    if is_training:
        ...
        augment_fn = functools.partial(
            ...
            only_augment_positive=dataset_config.only_augment_positive)
        dataset = dataset.map(
