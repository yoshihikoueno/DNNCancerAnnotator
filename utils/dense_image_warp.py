  ...
  with ops.name_scope(name):
    ...
    with ops.control_dependencies([
        check_ops.assert_less_equal(
            math_ops.cast(batch_size * depth * height * width,
    ...
    # batch, z, y, and x coordinates are pulled into the first dimension.
    # Then we gather. Finally, we reshape the output back. It's possible this
    # code would be made simpler by using array_ops.gather_nd.
    def gather(z_coords, y_coords, x_coords, name):
      with ops.name_scope('gather-' + name):
        linear_coordinates = (
          batch_offsets + z_coords * height * width + y_coords * width
          + x_coords)
    ...
    top_left_back = gather(floors[0], floors[1], floors[2], 'top_left_back')
    top_right_back = gather(floors[0], ceils[1], floors[2], 'top_right_back')
    bottom_left_back = gather(ceils[0], floors[1], floors[2],
                              'bottom_left_back')
    bottom_right_back = gather(ceils[0], ceils[1], floors[2],
                               'bottom_right_back')

    top_left_front = gather(floors[0], floors[1], ceils[2], 'top_left_front')
    top_right_front = gather(floors[0], ceils[1], ceils[2], 'top_right_front')
    bottom_left_front = gather(ceils[0], floors[1], ceils[2],
                               'bottom_left_front')
    bottom_right_front = gather(ceils[0], ceils[1], ceils[2],
                                'bottom_right_front')

    # now, do the actual interpolation
    with ops.name_scope('interpolate'):
      interp_top_back = (alphas[2] * (top_right_back - top_left_back)
                         + top_left_back)
      interp_bottom_back = (alphas[2] * (bottom_right_back - bottom_left_back)
                            + bottom_left_back)
      interp_back = (alphas[1] * (interp_bottom_back - interp_top_back)
                     + interp_top_back)

      interp_top_front = (alphas[2] * (top_right_front - top_left_front)
                          + top_left_front)
      interp_bottom_front = (alphas[2] * (
        bottom_right_front - bottom_left_front) + bottom_left_front)
      interp_front = (alphas[1] * (interp_bottom_front - interp_top_front)
                      + interp_top_front)

      interp = alphas[0] * (interp_front - interp_back) + interp_back
...
def dense_image_warp(image, flow, name='dense_image_warp'):
  ...
  images[b, k - flow[b, k, j, i, 0], j - flow[b, k, j, i, 1],
  i - flow[b, k, j, i, 2], c].
  The locations specified by this formula do not necessarily map to an int
  index. Therefore, the pixel value is obtained by bilinear
  interpolation of the 4 nearest pixels around
  (b, k - flow[b, k, j, i, 0], j - flow[b, k, j, i, 1],
  i - flow[b, k, j, i, 2]). For locations outside
  of the image, we use the nearest pixel values at the image boundary.
  Args:
    image: 5-D float `Tensor` with shape `[batch, depth, height, width, channels]`.
    flow: A 5-D float `Tensor` with shape `[batch, depth, height, width, 3]`.
    name: A name for the operation (optional).
